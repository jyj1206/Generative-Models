...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a VQGAN model (VQ regularization).")
...
def build_dataloaders(configs):
    ...
    sampler = DistributedSampler(train_dataset, shuffle=True) if configs.get("distributed", False) else None
...
def main():
    ...
    if start_epoch >= num_epochs:
        if is_main():
            print(f"Resume checkpoint already reached target epochs: start_epoch={start_epoch}, num_epochs={num_epochs}")
        cleanup_ddp()
        return
    ...
    for epoch in range(start_epoch + 1, num_epochs + 1):
        ...
        for inputs, _ in pbar:
            ...
            outputs, reg_losses = model(inputs)
            ...
            if disc_factor > 0.0:
                disc_to_use.eval()
                disc_to_use.requires_grad_(False)

                disc_fake_pred = disc_to_use(outputs)
                g_loss = discriminator_criterion.generator_loss(disc_fake_pred)
                d_weight = calculate_adaptive_weight(
                    nll_loss,
                    g_loss,
                    disc_weight,
                    model_to_use.decoder_conv_out.weight,
                )

                disc_to_use.requires_grad_(True)
                disc_to_use.train()
        ...
        if distributed:
            ...
            avg_train_loss_tensor /= dist.get_world_size()

            dist.all_reduce(avg_disc_train_loss_tensor, op=dist.ReduceOp.SUM)
            avg_disc_train_loss_tensor /= dist.get_world_size()
        ...
        if has_test_loader and is_main():
            ...
            with torch.no_grad():
                for test_inputs, _ in test_loader:
                    ...
                    outputs, reg_losses = model_to_use(test_inputs)

                    test_loss = recon_criterion(outputs, test_inputs)
                    regularization_loss = codebook_weight * (
                        reg_losses["codebook_loss"] + commitment_weight * reg_losses["commitment_loss"]
                    )
        ...
        last_avg_disc_train_loss = avg_disc_train_loss
